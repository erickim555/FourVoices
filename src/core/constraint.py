...
### ERIC IMPORTS ###
from Grader.grader import grade, grade_debug, partial_grade
import core.config
...
class Problem(object):
    ...
    def removeVariable(self, variable):
        if variable not in self._variables:
            raise ValueError, "Tried to remove a variable that doesn't exist %s" % repr(variable)
        else:
    ...
    def replaceVariable(self, variable, domain):
        ...
        """
        if variable not in self._variables:
            raise ValueError, "Tried to replace a variable that wasn't present %s" % \
                              repr(variable)
        if type(domain) in (list, tuple):
            domain = Domain(domain)
        elif isinstance(domain, Domain):
...
class BacktrackingSolver(Solver):
    ...
    def getSolutionIter(self, domains, constraints, vconstraints):
        ##### Variable for my attempt to prune search space
        best_grade = -10000000000000
        chords = core.config.chords
        harmonies = core.config.harmonies
        partial_grades = {}
        #####
        ...
        while True:
            ...
            for item in lst:
                ...
                if core.config.debug == 1:      # grade_debug returns a tuple of things
            ...
            # We have a variable. Do we have any values left?
                ...
                # or not we should continue to pursue the course at all
                ###
